 ...
 * @brief The {@code DisplayEditContactController} the user interface for editing an existing contact
 * <p>
 * The {@code DisplayEditContactController} provides the logic required to enable the user
 * to modify the datails of an existing contact via GUI
 * </p>
 *
 * <p>
 * This controller is designed to interact with an FXML user interface; it is linked to an
 * FXML file called "EditView.fxml" that is located in the "it.unisa.diem.gruppo9.interfaces.view" package.
 * The "@FXML" annotation can bind the UI components to the methods of this class
 * </p>
 *
 * <p>
 ...
 *
 * <p>
 * The editing functionality involves uptading contact information such as: name, surname,phoneNumber and email adresses;
 * The controller also give the user the opportunity to navigate back to the contac manager view using the {@code ChangeView}.
 * </p>
 *
 * @note Input validation and error handling
 * for editing contacts details will be impelemented in future to ensure data integrity
 *
 * @see ChangeView.java
...
public class DisplayEditContactController extends BaseController implements ContactManagerAware{
    ...
    /**Handles the action of editing the contact details when the "editButton" is clicked 
